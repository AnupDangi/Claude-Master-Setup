...
`${CLAUDE_PLUGIN_ROOT}/docs/EVALUATION.md`, `${CLAUDE_PLUGIN_ROOT}/docs/AI_OS.md`.

Capability protocol: `${CLAUDE_PLUGIN_ROOT}/docs/CAPABILITY_ORCHESTRATION.md`. When useful, spawn up to
...
1. **Tests** — `bash ${CLAUDE_PLUGIN_ROOT}/scripts/validate.sh` → `GATE: GREEN` or `GATE: RED`. Quote
   coverage only if the runner prints it.
2. **Iterations** — prefer loop-driven count from
   `bash ${CLAUDE_PLUGIN_ROOT}/scripts/loop-event.sh summary` → `loop_commits`. Also report git commit
...
3. **Documentation completeness** — filled vs placeholder for template docs in
   `.master/docs/` (`API.md`, `ARCHITECTURE.md`, `CODING_STANDARDS.md`, `DATABASE.md`,
...
```bash
bash ${CLAUDE_PLUGIN_ROOT}/scripts/write-scorecard.sh '{"tests":"GREEN|RED","iterations":N,"git_commits":N,"docs_filled":N,"docs_total":N,"manual_interventions":N,"notes":"..."}'
```

This updates `.master/state/last_scorecard.json` for the orchestrator's SELECT bias.
