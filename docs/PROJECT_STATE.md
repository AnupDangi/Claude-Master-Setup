...
(Decision 001/003). Milestones 0–2 (objective eval), 4 (capability orchestration),
**5 (AI OS control plane)**, and the plugin-packaging slice of Milestone 3
(Decision 005) are complete. Build-effort dial (Decision 004), a complexity-aware
ceremony dial (Decision 006), and a **shared-framework distribution model**
(Decision 007) round out the current shape: every install path (`--global`,
...
- Engineered loop with two human gates; hard validation; reviewer + security
  ...
  medium/large — Decision 006); retry cap and one-COMMIT default budget.
...
- Capability orchestration (Decision 003): local skill discovery, hierarchical caps,
...
- Build-effort dial (Decision 004): `fast|standard|rigorous` from PRD/PTR; review and
  security remain mandatory on all tiers.
- Plugin packaging (Decision 005): `.claude-plugin/plugin.json` (`master`) +
...
- Ceremony dial (Decision 006): `planner` skipped for a strict 5-condition
...
- **Shared-framework model (Decision 007):** `.master/`-only project footprint on
...
- Fixed: `.github/workflows` no longer leaks into consumer projects on
  `--local` installs.
- Lean commands: `/bootstrap` scaffolds `.master/`; `/pause` + `/decide` added;
  `/init` and `/ship` deleted entirely (no deprecation stubs).
...
5. Watch real usage of the ceremony dial (Decision 006) and the shared-framework
   model (Decision 007) — tighten either if real use surfaces a gap.
...
- Full Scheduler, benchmarks, and subjective metrics remain explicitly
  unimplemented.
- Decision 006's inline-plan path removes the independent planner opinion for
...
- Running the npm-installed framework *and* the `master` plugin on the same
  machine double-fires hooks (Decision 007) — the two paths are meant to be
